...
CoinAPI symbol-metadata endpoint. Its output is terminal-only, contains no credential, and proves
only authenticated access plus the fixed Spot BTC/USDT symbol identity. It neither requests
historical OHLCV nor writes a CSV, report, manifest or any other artifact.
